...
int point_inside_polygon(const Point<T> &q, vector<Point<T>> p) {
	...
	for (int i = 0; i < p.size(); i++) {
		if (p[i].y <= q.y) { // Candidate for upward edge.
			if (q.y < p[(i + 1) % p.size()].y and ((p[i] - q) ^ (p[(i + 1) % p.size()] - q)) > static_cast<T>(0)) { // Valid upward edge and to the right of Q.
				wn++;
			}
		}
		else { // Candidate for downward edge.
			if (q.y >= p[(i + 1) % p.size()].y and ((p[i] - q) ^ (p[(i + 1) % p.size()] - q)) < static_cast<T>(0)) { // Valid downward edge and to the right of Q.
				wn--;
			}
		}
	}

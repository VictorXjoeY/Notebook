...
pair<Point2D<T>, Point2D<T>> segment_segment_distance(const Point2D<U> &a, const Point2D<U> &b, const Point2D<U> &c, const Point2D<U> &d) {
    ...
    vector<pair<Point2D<T>, Point2D<T>>> ans = {
        {static_cast<Point2D<T>>(a), ra}, {static_cast<Point2D<T>>(b), rb}, {static_cast<Point2D<T>>(c), rc}, {static_cast<Point2D<T>>(d), rd}};

    /* O(1) - Compares pairs of points by their distance. */
    function<bool(const pair<Point2D<T>, Point2D<T>>, const pair<Point2D<T>, Point2D<T>>)> comp =
        [](const pair<Point2D<T>, Point2D<T>> &ca, const pair<Point2D<T>, Point2D<T>> &cb) { return dist(ca.first, ca.second) < dist(cb.first, cb.second); };

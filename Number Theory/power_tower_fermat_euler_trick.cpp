...
long long power_tower(long long a, long long n) {
    ...
    // We want to calculate a^x mod n for a big x.
    // We can write it as a^k * (a^(x - k) mod n / cf) mod n.
    // Given a k such that a^k mod cf = 0.
    long long phi = euler_phi(n / cf);
    return (fast_exp(a, cf, n) * fast_exp(a, (power_tower(a, phi) - cf % phi + phi) % phi, n / cf)) % n;
}
